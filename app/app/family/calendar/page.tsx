...
export default async function FamilyCalendarPage({
  ...
  return (
    <div>
      ...
      <nav aria-label="Período carregado" className="mt-5 grid grid-cols-4 rounded-2xl bg-[#e7effb] p-1">
        {periodOptions.map((option) => <Link key={option.value} href={`/app/family/calendar?month=${monthKey}&period=${option.value}`} aria-current={period === option.value ? "page" : undefined} className={`rounded-xl px-1 py-2.5 text-center text-[9px] font-extrabold transition ${period === option.value ? "bg-white text-[#1768c5] shadow-sm" : "text-[#68809e]"}`}>{option.label}</Link>)}
      </nav>

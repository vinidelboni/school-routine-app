  ...
  return (
    <div>
      ...
      <section className="mt-7 grid items-start gap-5 xl:grid-cols-[.82fr_1.18fr]">
        <form action={createOccurrence} className="rounded-2xl border border-[#dce6f2] bg-white p-5">
          <span className="text-[10px] font-extrabold tracking-[.12em] text-[#386b9f]">NOVO REGISTRO</span>
          ...
          <div className="mt-5 grid gap-4">
            ...
            <SubmitButton idleLabel="Registrar ocorrência" pendingLabel="Registrando..." className="rounded-xl bg-[#0759bd] px-4 py-3 text-xs font-bold text-white" />
          </div>
        </form>
        <div className="rounded-2xl border border-[#dce6f2] bg-white p-5">
          ...
          <div className="mt-5 grid gap-3">
            {occurrences?.map((occurrence) => {
              ...
              return (
                <article key={occurrence.id} className={`rounded-xl border p-4 ${occurrence.severity === "urgent" ? "border-[#d99a8d] bg-[#fff6f3]" : "border-[#e3eaf2]"}`}>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-xs leading-5 text-[#536b84]">{occurrence.description}</p>
                      <p className="mt-2 text-[10px] leading-4 text-[#61758d]"><strong>Providências:</strong> {occurrence.actions_taken}</p>
                    </div>
            ...
            {!occurrences?.length ? <div className="rounded-xl border border-dashed border-[#dce6f2] p-8 text-center text-xs text-[#6f8299]"><AlertTriangle className="mx-auto mb-2" size={22} /> Nenhuma ocorrência registrada.</div> : null}
          </div>
        </div>
      </section>
    </div>
...
function Status({ status, acknowledged, total }: { status: string; acknowledged: number; total: number }) {
  if (status === "internal") return <span className="flex items-center gap-1 rounded-full bg-[#eef0ed] px-2.5 py-1 text-[9px] font-bold text-[#61758d]"><Clock3 size={11} /> Interno</span>;
  return (

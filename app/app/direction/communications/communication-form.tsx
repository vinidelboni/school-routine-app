...
export function CommunicationForm({
  ...
  return (
    <form action={createCommunication} className="rounded-2xl border border-[#dfe1d9] bg-white p-5">
      ...
      <div className="mt-5 grid gap-4">
        <label className="grid gap-1.5 text-xs font-bold">
          Tipo
          <select name="kind" className="rounded-xl border border-[#dfe1d9] bg-white px-3 py-3 font-normal" required>
        ...
        <label className="grid gap-1.5 text-xs font-bold">
          ...
          <select
            ...
            className="rounded-xl border border-[#dfe1d9] bg-white px-3 py-3 font-normal"
          >
        ...
        {scope === "classroom" ? (
          <label className="grid gap-1.5 text-xs font-bold">
            Turma
            <select name="classroomId" className="rounded-xl border border-[#dfe1d9] bg-white px-3 py-3 font-normal" required>
        ...
        {scope === "child" ? (
          <label className="grid gap-1.5 text-xs font-bold">
            Criança
            <select name="childId" className="rounded-xl border border-[#dfe1d9] bg-white px-3 py-3 font-normal" required>
              <option value="">Selecione</option>
        ...
        <label className="grid gap-1.5 text-xs font-bold">
          Mensagem
          <textarea name="body" minLength={3} maxLength={2000} rows={5} className="resize-none rounded-xl border border-[#dfe1d9] px-3 py-3 font-normal leading-5" placeholder="Escreva as informações que a família precisa receber." required />
        </label>
        <label className="grid gap-1.5 text-xs font-bold">
          Data relacionada <span className="font-normal text-[#7c8680]">(opcional)</span>
          <input type="date" name="eventDate" className="rounded-xl border border-[#dfe1d9] px-3 py-3 font-normal" />
        </label>
        <SubmitButton
          idleLabel="Publicar para as famílias"
          pendingLabel="Publicando..."
          className="rounded-xl bg-[#315645] px-4 py-3 text-xs font-bold text-white"
        />
      </div>
    </form>
  );

    ...
    >
      ...
      <fieldset className="mt-5">
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          {childOptions.map((child) => (
            <label
              key={child.id}
              className="flex cursor-pointer items-start gap-3 rounded-xl border border-[#e5e5df] p-3 text-xs has-checked:border-[#315645] has-checked:bg-[#eef3ef]"
            >
              <input
                type="checkbox"
                name="childIds"
                value={child.id}
                className="mt-0.5 accent-[#315645]"
              />
              <span>
                <strong className="block">{child.name}</strong>
                <small className="text-[#7c8680]">{child.classroom}</small>
              </span>
            </label>
          ))}
        </div>
      ...
      <fieldset
        ...
        className={`mt-5 rounded-xl border p-4 ${
          grantsAccess
            ? "border-[#dfe1d9] bg-[#fafbf9]"
            : "border-[#ecece7] bg-[#f4f4f0] opacity-60"
        }`}
      >
        <legend className="px-1 text-[10px] font-extrabold uppercase tracking-[.08em] text-[#557164]">
        ...
        <label className="mt-4 flex items-start gap-3 border-t border-[#e2e6e2] pt-4 text-xs">
          ...
          <span>
            ...
            <small className="mt-1 block leading-4 text-[#69746f]">

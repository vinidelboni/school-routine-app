...
export default async function DirectionPhotosPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div>
    ...
    <section className="mt-7 grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="rounded-2xl border border-[#dce6f2] bg-white p-5"><span className="flex items-center gap-2 text-[10px] font-extrabold tracking-[.12em] text-[#386b9f]"><Camera size={15} /> PUBLICAÇÕES</span><h2 className="mt-1 font-[var(--font-display)] text-2xl font-semibold">Auditoria recente</h2><div className="mt-4 grid gap-3">{photos?.map((photo) => { const profile = Array.isArray(photo.profiles) ? photo.profiles[0] : photo.profiles; return <article key={photo.id} className="rounded-xl border border-[#e3eaf2] p-4"><strong className="block text-sm">{photo.caption}</strong><small className="mt-1 block text-[#6f8299]">{photo.photo_children.length} criança(s) · {profile?.full_name}</small><span className="mt-2 flex items-center gap-1 text-[9px] font-bold text-[#0759bd]"><ShieldCheck size={12} /> Autorizações verificadas</span></article>; })}{!photos?.length ? <p className="rounded-xl border border-dashed border-[#dce6f2] p-6 text-center text-xs text-[#6f8299]">Nenhuma foto publicada.</p> : null}</div></div>
    </section>
  </div>;
}
